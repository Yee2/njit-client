...
START_AUTHENTICATION:
    {
        ...
        for (;;)
        {
            ...
            { // 处理认证失败信息
                ...
                { // 输出错误提示消息
                    memcpy(error_msg,&captured[24],msgsize);
                    error_msg[msgsize] = 0x00;
                    if(strncmp(&captured[24],"E63018",6)==0){
                        DPRINTF("%s\n",error_msg);
                        return 0;
                    }
                    if(strncmp(&captured[24],"E63034",6)==0){
                        DPRINTF("%s\n",error_msg);
                        return 0;
                    }
                    if(strncmp(&captured[24],"E63025",6)==0){
                        DPRINTF("%s\n",error_msg);
                        return 0;
                    }
                    if(strncmp(&captured[24],"E63013",6)==0){
                        DPRINTF("%s\n",error_msg);
                        return 0;
                    }
                    if(strncmp(&captured[24],"E63032",6)==0){
                        DPRINTF("%s\n",error_msg);
                        return 0;
                    }
                    fprintf(stderr, "%s\n", msg);
